...
## Key Features
- **SEO**: `SEO.tsx` component (title/description/canonical/og/schema.org via useEffect). Find, BuilderProfile, SubProfile, RealtorPublicCard all emit dynamic meta tags + JSON-LD. TaxProPublicProfile public page at `/tax-pro/:id` (no auth). CityPage at `/contractors/:cityState` for programmatic city landing pages. Sitemap at `/sitemap.xml` now includes realtors, tax-pros and city pages. `robots.txt` updated.
- **Legal Consent System**: Full legal consent system — DB columns `terms_accepted_at`, `terms_version`, `terms_accepted_ip` on `users` table. Backend validates `termsAccepted: true` and saves timestamp + version (`CURRENT_TERMS_VERSION = "v1.0"`) + IP on registration. Register.tsx has explicit checkbox (required, disables submit button). `LegalFooter` reusable component in `BuilderLayout` and `RealtorLayout` sidebars. Fixed broken `href="#"` links in RealtorPublicCard.
- **Realtor Portal**: Full portal at `/realtor` with dashboard, profile editor, trusted pros management, and public card (`/realtor/:id`). New DB columns: `brokerage`, `license_number`, `phone` on users; `realtor_trusted_pros` table. Backend routes: `GET/PATCH /api/realtor/profile`, `GET/POST/DELETE /api/realtor/trusted-pros[/:id]`, `GET /api/realtor/public/:id`. Public card supports vCard download + Web Share API.
