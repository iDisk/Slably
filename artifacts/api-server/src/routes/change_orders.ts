...
import { db, changeOrdersTable, projectsTable } from "@workspace/db";
import {
...
router.patch("/projects/:projectId/change-orders/:id", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  if (parsed.data.amount !== null && parsed.data.amount !== undefined) updateData.amount = String(parsed.data.amount);
...
router.post("/projects/:projectId/change-orders/:id/approve", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  if (user.role !== "client" && user.role !== "builder") {
    res.status(403).json({ error: "Forbidden" });
    return;
  }
